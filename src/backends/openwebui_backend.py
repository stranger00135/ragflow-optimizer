...
This backend implements the RAGBackend interface for OpenWebUI,
allowing the RAG optimizer to work with OpenWebUI knowledge bases.
...
class OpenWebUIBackend(RAGBackend):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.base_url or not self.api_key:
            raise BackendError("OpenWebUI backend requires base_url and api_key in config")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise BackendError(f"Request failed after {self.max_retries} retries: {e}")
                time.sleep(2 ** attempt)

        raise BackendError("Unexpected error in request handling")
    ...
    def create_collection(self, name: str, config: Dict[str, Any]) -> str:
        ...
        if not kb_id:
            raise BackendError("No knowledge base ID returned from OpenWebUI")
    ...
    def _wait_for_file_processing(
        ...
        Raises:
            BackendError: If processing fails or times out
        """
        start = time.time()
        while time.time() - start < timeout:
            try:
                ...
                if status in ("completed", "indexed", ""):
                    ...
                    raise BackendError(f"File processing failed: {file_id}")
        ...
        raise BackendError(f"File processing timeout: {file_id}")
    ...
    def get_collection_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Knowledge base dict if found, otherwise None
        """
        try:
            response = self._request("GET", "/api/v1/knowledge/")
            knowledges = response if isinstance(response, list) else response.get("data", [])
